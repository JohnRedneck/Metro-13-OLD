...
/datum/game_mode/proc/handle_latejoin(var/mob/living/carbon/human/character)
	if(character.mind)
		character.mind.generate_goals(character.mind.assigned_role)
...
/datum/game_mode/proc/process_autoantag()
	...
	while(usable_templates.len)
		...
		if(spawn_antag.attempt_auto_spawn())
			message_admins("[uppertext(name)]: Auto-added a new [spawn_antag.rank_text].")
			message_admins("There are now [spawn_antag.get_active_antag_count()]/[spawn_antag.cur_max] active [spawn_antag.rank_text_plural].")
