...
/obj/machinery/computer/robotics/OnTopic(var/mob/user, href_list)
	...
	// Remotely hacks the cyborg. Only antag AIs can do this and only to linked cyborgs.
		...
		// Antag AI checks
		if(!istype(user, /mob/living/silicon/ai) || !(user.mind.special_rank && user.mind.original == user))
			to_chat(user, "<span class='warning'>Access Denied</span>")
			return TOPIC_HANDLED
...
/obj/machinery/computer/robotics/proc/get_cyborgs(var/mob/operator)
	...
	for(var/mob/living/silicon/robot/R in GLOB.silicon_mob_list)
		...
		if(operator && istype(operator, /mob/living/silicon/ai) && (R.connected_ai == operator) && (operator.mind.special_rank && operator.mind.original == operator))
