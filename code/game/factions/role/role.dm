/datum/role
	...
	var/title = "NOPE"
	//Job access. The use of minimal_access or access is determined by a config setting: config.roles_have_minimal_access
	//var/list/minimal_access = list()      // Useful for servers which prefer to only have access given to the places a role absolutely needs (Larger server population)
	...
	var/role_flag = 0
	var/team_flag = 0
	var/total_positions = 0               // How many players can be this role
	var/spawn_positions = 0               // How many players can spawn in as this role
	var/current_positions = 0             // How many players have this role
	var/availablity_chance = 100          // Percentage chance role is available each round
	...
	var/team = null                       // Does this position have a team tag?
	var/lead_position = 0                 // Is this position a leader?
	var/minimum_character_age = 0
	var/ideal_character_age = 30
	var/create_record = 1                 // Do we announce/make records for people who spawn on this role?

	var/account_allowed = 1               // Does this role type come with a station account?
	var/economic_power = 2                // With how much does this role modify the initial account amount?
	...
	var/latejoin_at_spawnpoints           //If this role should use roundstart spawnpoints for latejoin (offstation roles etc)

	var/hud_icon						  //icon used for Sec HUD overlay

	var/min_skill = list()				  //Minimum skills allowed for the role. List should contain skill (as in /decl/hierarchy/skill path), with values which are numbers.
	var/max_skill = list()				  //Maximum skills allowed for the role.
	var/skill_points = 16				  //The number of unassigned skill points the role comes with (on top of the minimum skills).
	...
	var/defer_roundstart_spawn = FALSE // If true, the role will be put off until all other roles have been populated.
...
/datum/role/dd_SortValue()
	return title

/datum/role/proc/equip(var/mob/living/carbon/human/H, var/alt_title, var/datum/mil_branch/branch, var/datum/mil_rank/grade)
/*
...
/datum/role/proc/get_outfit(var/mob/living/carbon/human/H, var/alt_title, var/datum/mil_branch/branch, var/datum/mil_rank/grade)
	...
	. = outfit_by_type(.)
/*
/datum/role/proc/setup_account(var/mob/living/carbon/human/H)
...
/datum/role/proc/equip_preview(mob/living/carbon/human/H, var/alt_title, var/datum/mil_branch/branch, var/datum/mil_rank/grade, var/additional_skips)
	var/decl/hierarchy/outfit/outfit = get_outfit(H, alt_title, branch, grade)
	if(!outfit)
		return FALSE
...
/datum/role/proc/available_in_days(client/C)
	if(C && config.use_age_restriction_for_roles && isnull(C.holder) && isnum(C.player_age) && isnum(minimal_player_age))
		return max(0, minimal_player_age - C.player_age)
	return 0

/datum/role/proc/apply_fingerprints(var/mob/living/carbon/human/target)
	if(!istype(target))
		return 0
...
/datum/role/proc/apply_fingerprints_to_item(var/mob/living/carbon/human/holder, var/obj/item/item)
	item.add_fingerprint(holder,1)
	if(item.contents.len)
		for(var/obj/item/sub_item in item.contents)
			apply_fingerprints_to_item(holder, sub_item)

/datum/role/proc/is_position_available()
...
/datum/role/proc/get_join_link(var/client/caller, var/href_string, var/show_invalid_roles)
	if(is_available(caller))
		if(is_restricted(caller.prefs))
			if(show_invalid_roles)
...
// Only players with the role assigned and AFK for less than 10 minutes count as active
/datum/role/proc/check_is_active(var/mob/M)
	return (M.mind && M.client && M.mind.assigned_role == title && M.client.inactivity <= 10 * 60 * 10)

/datum/role/proc/get_active_count()
	var/active = 0
...
/datum/role/proc/is_species_allowed(var/datum/species/S)
	if(GLOB.using_map.is_species_role_restricted(S, src))
		return FALSE
...
// Don't use if the map doesn't use branches but roles do.
/datum/role/proc/get_branch_rank(var/datum/species/S)
...
/**
 *  Check if members of the given branch are allowed in the role
 *
 ...
 */
/datum/role/proc/is_branch_allowed(var/branch_name)
	if(!allowed_branches || !GLOB.using_map || !(GLOB.using_map.flags & MAP_HAS_BRANCH))
		return 1
	if(branch_name == "None")
		return 0
...
/**
 *  Check if people with given rank are allowed in this role
 *
 *  This proc should only be used after the global branch list has been initialized.
 *
 ...
 */
/datum/role/proc/is_rank_allowed(var/branch_name, var/rank_name)
...
//Returns human-readable list of branches this role allows.
/datum/role/proc/get_branches()
	var/list/res = list()
...
/datum/role/proc/get_ranks(branch)
	var/list/res = list()
...
/datum/role/proc/get_description_blurb()
	return ""

/datum/role/proc/get_role_icon()
	if(!SSroles.role_icons[title])
		var/mob/living/carbon/human/dummy/mannequin/mannequin = get_mannequin("#role_icon")
		dress_mannequin(mannequin)
		mannequin.dir = SOUTH
		var/icon/preview_icon = getFlatIcon(mannequin)
		preview_icon.Scale(preview_icon.Width() * 2, preview_icon.Height() * 2) // Scaling here to prevent blurring in the browser.
		SSroles.role_icons[title] = preview_icon
	return SSroles.role_icons[title]

/datum/role/proc/get_unavailable_reasons(var/client/caller)
...
/datum/role/proc/dress_mannequin(var/mob/living/carbon/human/dummy/mannequin/mannequin)
	mannequin.delete_inventory(TRUE)
	equip_preview(mannequin, additional_skips = OUTFIT_ADJUSTMENT_SKIP_BACKPACK)

/datum/role/proc/is_available(var/client/caller)
	if(!is_position_available())
		return FALSE
	if(roleban_isbanned(caller, title))
		return FALSE
...
/datum/role/proc/make_position_available()
	total_positions++

/datum/role/proc/get_roundstart_spawnpoint()
...
/datum/role/proc/get_spawnpoint(var/client/C)
	...
	if(!spawnpos)
		...
		warning("Could not find an appropriate spawnpoint for role [title].")
