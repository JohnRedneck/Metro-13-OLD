...
/datum/category_item/player_setup_item/background/records/content(var/mob/user)
	. = list()
	. += "<br/><b>Records</b>:<br/>"
	if(roleban_isbanned(user, "Records"))
...
/datum/category_item/player_setup_item/background/records/OnTopic(var/href,var/list/href_list, var/mob/user)
	if (href_list["set_public_record"])
		...
		if (!isnull(new_public) && !roleban_isbanned(user, "Records") && CanUseTopic(user))
		...
		if(!isnull(new_medical) && !roleban_isbanned(user, "Records") && CanUseTopic(user))
		...
		if(!isnull(new_general) && !roleban_isbanned(user, "Records") && CanUseTopic(user))
		...
		if(!isnull(sec_medical) && !roleban_isbanned(user, "Records") && CanUseTopic(user))
