...
/datum/nano_module/records/ui_interact(mob/user, ui_key = "main", datum/nanoui/ui = null, force_open = 1, state = GLOB.default_state)
	...
	if(active_record)
		...
		data["pic_edit"] = check_access(user, null) || check_access(user, null)
		...
		data["creation"] = check_access(user, null)
		data["dnasearch"] = check_access(user, null) || check_access(user, null)
		data["fingersearch"] = check_access(user, null)
