...
/proc/get_camera_access(var/network)
	...
	switch(network)
		...
		if(NETWORK_CRESCENT, NETWORK_ERT)
			return nullnt_specops
		if(NETWORK_MEDICAL)
			return null
		if(NETWORK_MINE)
			return null // Cargo office - all cargo staff should have access here.
		if(NETWORK_RESEARCH)
			return null
		*/
		if(NETWORK_THUNDER)
			return 0
	return 0
	//return null // Default for all other networks
...
/datum/nano_module/camera_monitor/proc/can_access_network(var/mob/user, var/network_access)
	...
	return 1//check_access(user, null) || check_access(user, network_access)
